...
public class TabellHaug<T extends Comparable<T>> {
	...
	private void reparerOpp() {
		int nyNodePos = antall - 1;
		int foreldrePos = (nyNodePos - 1)/2;
		T temp = data[nyNodePos];
		
		while(nyNodePos > 0 && data[foreldrePos].compareTo(temp) > 0) {
			data[nyNodePos] = data[foreldrePos];
			nyNodePos = foreldrePos;
			foreldrePos = (nyNodePos-1)/2;
		}
		data[nyNodePos] = temp;
	}
